...
export const constructUrl = ({ pageNumber, search, pathname }: ConstructUrlParams): string => {
    //Here we want to retain all of the existing filter params 
    //and just add another param for the page number
...
export const constructPrevOrNextUrl = ({
    ...
    pageCount,
    search,
    pathname }: ConstructPrevOrNextParams): { prevUrl: string, nextUrl: string } => {
